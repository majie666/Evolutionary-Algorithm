...
# build multivariate distribution 多变量正态分布
mean = tf.Variable(tf.random.normal([DNA_SIZE, ], 13., 1.), dtype=tf.float32)
...
make_kid = mvn.sample(N_POP) # sampling operation (N_POP,DNA_SIZE)
...
loss = -tf.reduce_mean(mvn.log_prob(tfkids)*tfkids_fit) # 尽量沿着好孩子的梯度方向及强度走，tf没有最大化，因此添加负号后最小化
train_op = tf.train.GradientDescentOptimizer(LR).minimize(loss) # apply gradients for mean and cov
